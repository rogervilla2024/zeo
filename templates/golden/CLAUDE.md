...
| Page anatomy | `homepage.hero` (standard/search/compact/none) + `homepage.aside` (right/left/none) + `header.search` + `feed` block + `directory:shelves` - the topic picks the anatomy (recipes.md A1-A6: story lead, booking funnel, marketplace, feed board, dense portal, reading room) |
